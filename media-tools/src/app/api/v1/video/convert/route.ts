...
const VALID_FORMATS = ["mp4", "webm", "avi", "mkv", "mov", "wmv", "flv", "m4v"];
  ...
  try {
    ...
    const task = await prisma.task.create({
      data: {
        type: "video-convert",
        inputFile: filePath,
        params: { format, resolution, codec },
        userId: auth.user?.id,
      },
    });

    // Add to queue
    await addJob("video-convert", {
      taskId: task.id,
      inputFile: filePath,
      format,
      resolution,
    ...
    console.error("API video convert error:", error);
    return NextResponse.json(
